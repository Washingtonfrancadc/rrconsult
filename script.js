...
function gerenciarIframeInjetado(exId, url) {
  ...
  setTimeout(() => {
    const wrapper = document.getElementById("wrapper-gif-elemento");
    if (wrapper) wrapper.classList.add("aberto");
  }, 50);
}
                        ...
                        ${b.exercicios
                          .map(
                            ...
                            <tr draggable="true" ondragstart="dragExercicioStart(event, ${bIdx}, ${eIdx})" ondragover="dragExercicioOver(event, ${bIdx}, ${eIdx})" ondragend="dragExercicioEnd(event)">
                                <td>${gerarSelectExerciciosPorMusculo(ex.nome, `onChangeExercicioSelect(this, ${bIdx}, ${eIdx})`)}</td>
...
function renderizarTrackCompleto() {
  ...
  const GAP = 10;
  const cardWidth = Math.floor((viewport.clientWidth - GAP) / 2);
...
function atualizarCarrossel() {
  ...
  const cardWidth =
    ...
    Math.floor(
      (document.getElementById("carrossel-viewport").clientWidth - 10) / 2,
    );
  track.style.transform =
    "translateX(-" + carrosselPaginaAtual * cardWidth + "px)";
